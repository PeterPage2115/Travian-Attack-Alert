...
test('Given a fixture, When backup runs, Then it publishes matching payloads and four integrity files', () => {
...
test('Given a valid backup, When rollback selects version-hash, Then it restores the matching triplet without changing package version', () => {
    const project = fixture();
    try {
...
for (const mutation of ['schemaVersion', 'version', 'releaseId', 'selector', 'path', 'digest', 'artifactSha256']) test(`Given a manifest with corrupt ${mutation}, When rollback runs, Then live files remain unchanged`, () => {
    const project = fixture();
    try {
        ...
        if (mutation === 'schemaVersion') manifest.schemaVersion = 2; else if (mutation === 'version') manifest.version = '6.1.1'; else if (mutation === 'releaseId') manifest.releaseId = 'taa-6.1.1'; else if (mutation === 'selector') manifest.selector = '6.1.1-deadbeef'; else if (mutation === 'path') manifest.files.script.path = '../script.txt'; else if (mutation === 'digest') manifest.files.metadata.sha256 = '0'.repeat(64); else manifest.artifactSha256 = '0'.repeat(64);
        fs.writeFileSync(manifestPath, `${JSON.stringify(manifest)}\n`); unchangedFailure(project, [selector]);
    } finally { fs.rmSync(project, { recursive: true, force: true }); }
});

for (const kind of ['script', 'metadata', 'moduleManifest']) test(`Given backup copy failure for ${kind}, When backup runs, Then no selector files are published`, () => {
    const project = fixture();
...
for (const missing of ['metadata.json', 'script.txt']) test(`Given missing live ${missing}, When rollback runs, Then validated backup restores the complete triplet`, () => {
    const project = fixture();
    try { const { selector, digest, dir } = selectorInfo(project); fs.rmSync(path.join(project, missing)); const result = run(project, 'rollback.cjs', [selector]); assert.equal(result.status, 0, result.stderr); assert.deepEqual(liveHashes(project), [digest, hash(path.join(dir, payloadName('metadata', selector))), hash(path.join(dir, payloadName('moduleManifest', selector)))]); }
    finally { fs.rmSync(project, { recursive: true, force: true }); }
});

for (const failure of [
    ...['script', 'metadata', 'moduleManifest'].map((kind) => ({ name: `swap-${kind}`, env: { TAA_FAIL_SWAP: kind } })),
...
]) test(`Given ${failure.name}, When rollback fails, Then startup recovery restores the original triplet`, () => {
    const project = fixture();
...
test('Given a current-version backup, When rollback receives bare hash8, Then it selects the current package version', () => {
    const project = fixture();
    try { const { digest } = selectorInfo(project); fs.writeFileSync(path.join(project, 'script.txt'), 'changed'); const result = run(project, 'rollback.cjs', [digest.slice(0, 8)]); assert.equal(result.status, 0, result.stderr); assert.equal(hash(path.join(project, 'script.txt')), digest); }
    finally { fs.rmSync(project, { recursive: true, force: true }); }
});

test('Given a swap failure, When rollback starts again, Then startup recovery restores the original matching triplet', () => {
    const project = fixture();
    try {
...
test('Given a crash after one old move, When rollback starts again, Then recovery restores the original triplet', () => {
    const project = fixture();
    try {
...
test('Given a crash after one target swap, When rollback starts again, Then recovery restores the original triplet', () => {
    const project = fixture();
    try {
        const { selector } = selectorInfo(project); const before = liveHashes(project);
...
test('Given a missing live metadata file and a crash after one old move, When rollback restarts, Then recovery and retry restore the validated backup triplet', () => {
    const project = fixture();
    try {
        ...
        assert.deepEqual(liveHashes(project), [digest, hash(path.join(dir, payloadName('metadata', selector))), hash(path.join(dir, payloadName('moduleManifest', selector)))]); assert.equal(fs.readdirSync(dir).filter((name) => /^\.rollback-journal-/.test(name)).length, 0);
    } finally { fs.rmSync(project, { recursive: true, force: true }); }
});
