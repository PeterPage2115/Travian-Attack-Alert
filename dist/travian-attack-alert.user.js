  ...
  "src/runtime.js"(exports2, module2) {
    (function() {
      ...
      const isNodeEnvironment = Boolean(
        typeof document === "undefined" && typeof location === "undefined" && typeof module2 !== "undefined" && module2.exports
      );
      ...
      async function runNameBackfill(ids, options = {}) {
        ...
        const fetchProfile = typeof options.fetchProfile === "function" ? options.fetchProfile : (id, signal2) => fetch("/profile/" + encodeURIComponent(id), { signal: signal2 });
      ...
      function inspectMappingStorage(rawValue, hostname) {
        ...
        const otherHosts = /* @__PURE__ */ new Set();
      ...
      function mergeSettingsBackupMap(current, imported, hostname, validateWorld) {
        ...
        for (const [rawHost, rawWorld] of Object.entries(source)) {
          ...
          const accepted = validateWorld(rawWorld, () => {
            invalid += 1;
          });
          ...
          if (host === worldKey) currentEntries += count;
          else otherHostEntries += count;
        }
      ...
      function buildSettingsBackup(input = {}) {
        ...
        for (const key of SETTINGS_BACKUP_DATA_KEYS) {
          ...
          data[key] = key === WEBHOOK_STORAGE_KEY ? (() => {
            const value = String(input.webhook === void 0 ? loadWebhookUrl() || "" : input.webhook);
            return value ? { action: "set", url: value } : { action: "clear" };
          })() : settingsBackupStorageValue(storage, key);
        }
      ...
      function inspectSettingsBackup(raw, hostname, current = {}) {
        ...
        try {
          ...
          return { ok: false, reason: "invalid-json", invalid: ["json"] };
        }
        ...
        const strictWorldMap = (key, validator) => {
          ...
          if (!source || typeof source !== "object" || Array.isArray(source)) {
            invalid.push(key);
            return { value: current[key], currentEntries: 0, otherHostEntries: 0 };
          }
          for (const rawHost of Object.keys(source)) {
            const host = normalizeHostname(rawHost);
            if (!host || !/^[a-z0-9.-]+$/.test(host) || host !== rawHost) {
              invalid.push(key);
              continue;
            }
        ...
        const validateMapping = (world, key, reject = () => invalid.push(key)) => {
          ...
          for (const [playerId, ids] of Object.entries(world)) {
            if (!/^\d+$/.test(playerId) || !Array.isArray(ids) || !ids.length || ids.some((id) => !validateDiscordUserId(id))) {
              reject();
              continue;
            }
        ...
        const validateSettingsWorld = (world, key, reject = () => invalid.push(key)) => {
          ...
          for (const field of ["attackThreshold", "raidThreshold", "normalMax", "highMax"]) if (Object.prototype.hasOwnProperty.call(world, field)) {
            ...
            if (typeof value !== "number" || !Number.isFinite(value) || !Number.isInteger(value) || value < 1 || field.endsWith("Threshold") && value > 999) reject();
        ...
        const validateSimple = (expected) => (world, key, reject = () => invalid.push(key)) => {
          ...
          for (const [id, value] of Object.entries(world)) {
            if (!expected(value) || String(id).trim() === "") reject();
            else accepted[id] = value;
          }
          return accepted;
        };
        const plans = {
        ...
        if (Object.prototype.hasOwnProperty.call(data, DISCORD_CONFIG_STORAGE_KEY)) {
          ...
          if (!source || typeof source !== "object" || Array.isArray(source)) invalid.push(DISCORD_CONFIG_STORAGE_KEY);
          else for (const field of Object.keys(source)) {
            if (!["roleId", "leaveRoleId"].includes(field)) {
              invalid.push(DISCORD_CONFIG_STORAGE_KEY);
              continue;
            }
            if (source[field] !== null && source[field] !== "" && !validateDiscordRoleId(source[field])) invalid.push(DISCORD_CONFIG_STORAGE_KEY);
            else roles[field] = source[field] === "" ? null : source[field];
          }
        }
        ...
        if (Object.prototype.hasOwnProperty.call(data, WEBHOOK_STORAGE_KEY)) {
          ...
          if (typeof source === "string") {
            const url = validateWebhookUrl(source);
            if (!url) invalid.push(WEBHOOK_STORAGE_KEY);
            else webhook = { action: "set", url };
      ...
      function applySettingsBackup(raw, options = {}) {
        ...
        const current = {};
        try {
          for (const key of SETTINGS_BACKUP_DATA_KEYS) current[key] = key === WEBHOOK_STORAGE_KEY ? options.webhook ? options.webhook.value || "" : validateWebhookUrl(readGm()) || "" : settingsBackupStorageValue(storage, key);
        } catch (error) {
          return { ok: false, reason: "preimage-failed" };
        }
        const plan = inspectSettingsBackup(raw, hostname, current);
        if (!plan.ok) return plan;
        const original = /* @__PURE__ */ new Map();
        try {
          for (let index = 0; index < (storage ? storage.length : 0); index += 1) {
            const key = storage.key(index);
            if (key !== null) original.set(key, storage.getItem(key));
          }
        } catch (error) {
          return { ok: false, reason: "preimage-failed" };
        }
        ...
        const setWebhook = options.setWebhook || ((value) => {
          if (options.webhook) webhook.value = value;
          else if (typeof gm.set === "function") gm.set(value);
          else if (!saveWebhookUrl(value)) throw new Error("webhook-write-failed");
        });
        const clearWebhook = options.clearWebhook || (() => {
          if (options.webhook) webhook.value = null;
          else if (typeof gm.remove === "function") gm.remove();
        ...
        try {
          for (const [key, item] of [[MAPPING_STORAGE_KEY, plan.plans.mappings], [SETTINGS_STORAGE_KEY, plan.plans.settings], [MUTED_PLAYERS_STORAGE_KEY, plan.plans.muted], [PLAYER_NAMES_STORAGE_KEY, plan.plans.names], [ROSTER_STORAGE_KEY, plan.plans.roster]]) if (Object.prototype.hasOwnProperty.call(plan.backup.data, key)) {
            const result = writeVerifiedJson(storage, key, item.value);
            if (!result.ok) throw result;
          }
          if (Object.prototype.hasOwnProperty.call(plan.backup.data, DISCORD_CONFIG_STORAGE_KEY)) {
            const result = writeVerifiedJson(storage, DISCORD_CONFIG_STORAGE_KEY, plan.roles);
            if (!result.ok) throw result;
          }
          if (plan.webhook.action === "set") setWebhook(plan.webhook.url);
          else if (plan.webhook.action === "clear") clearWebhook();
          ...
          try {
            for (const key of [.../* @__PURE__ */ new Set([...original.keys(), ...SETTINGS_BACKUP_DATA_KEYS])]) {
              if (original.has(key)) storage.setItem(key, original.get(key));
              else storage.removeItem(key);
            }
            if (options.webhook) webhook.value = oldWebhook || null;
            else if (gmWasPresent && typeof gm.set === "function") gm.set(oldWebhookRaw);
            else if (gmWasPresent && typeof GM_setValue === "function") GM_setValue(WEBHOOK_STORAGE_KEY, oldWebhookRaw);
            else if (!gmWasPresent && typeof gm.remove === "function") gm.remove();
            else if (!gmWasPresent && typeof GM_deleteValue === "function") GM_deleteValue(WEBHOOK_STORAGE_KEY);
          } catch (rollbackError) {
      ...
      function buildDiagnosticsPanelModel(lastFailure) {
        ...
        if (lastFailure.memberRows !== void 0 || lastFailure.rows !== void 0 || lastFailure.icons !== void 0) {
      ...
      function buildOperationalStateLines(input = {}) {
        ...
        const leaseValue = leaseGeneration === null && monitorGeneration === null ? "no lease held (standby)" : "lease generation " + (leaseGeneration === null ? "—" : leaseGeneration) + " · monitor generation " + (monitorGeneration === null ? "—" : monitorGeneration);
        const baselineValue = !hasObservation || baselinePlayers === null ? "not established — the first accepted scan commits it silently (no historical flood)" : "established — " + baselinePlayers + (baselinePlayers === 1 ? " player" : " players");
        const deliveryValue = !webhookConfigured ? "webhook missing — configuration required; queue preserved" : failed > 0 || uncertain > 0 ? "webhook configured — " + failed + " failed / " + uncertain + " uncertain; recover from the Tampermonkey menu" : "webhook configured — 0 failed / 0 uncertain";
        return [
      ...
      function buildPlayerWorkspaceReadModel(input) {
        ...
        const rosterStatusText = authoritative ? "" : cachedCount ? "Live roster unavailable — showing last accepted roster (" + cachedCount + " players). Reason: " + reason : "Live roster unavailable — no accepted roster is stored. Reason: " + reason;
      ...
      function monitorLegacyWorldKeyed(legacy, world) {
        return monitorLegacyWorldEntry(legacy && legacy.pending, world) !== void 0 || monitorLegacyWorldEntry(legacy && legacy.inFlight, world) !== void 0 || monitorLegacyWorldEntry(legacy && legacy.failed, world) !== void 0;
      }
      ...
      function reconcileMonitorLegacyQueuesV1(world, loadedEnvelope, options = {}) {
        ...
        const needsCleanup = monitorPendingMapNeedsCleanup(legacy.pending, hostname) || monitorFailedMapNeedsCleanup(legacy.failed, hostname);
...
${entry.line}`;
      ...
      const SCAN_CYCLE_DEADLINE_MS = 15e3;
          ...
          if (adminTab === "overview") {
            ...
            const draw = () => {
              ...
              for (const row of paged.rows) {
                ...
                const confirm2 = makeButton("Confirm removal for profile " + row.id, "taa-player-remove-confirm-" + row.id, () => {
                ...
                append(confirmActions, confirm2);
            ...
            const previewBackup = (backup) => {
              ...
              if (preview.data && preview.data[WEBHOOK_STORAGE_KEY]) {
                const webhook = preview.data[WEBHOOK_STORAGE_KEY];
                preview.data[WEBHOOK_STORAGE_KEY] = typeof webhook === "string" ? maskWebhookUrl(webhook) : webhook.action === "set" ? { action: "set", url: maskWebhookUrl(webhook.url) } : { action: "clear" };
              }
              settingsPreview.value = JSON.stringify(preview, null, 2);
            };
            const applySettingsImport = (raw) => {
              if (!mutationAllowed()) {
                setFeedback("Standby is read-only; settings import is disabled.", true);
                return;
              }
              ...
              if (!plan.ok) {
                setFeedback("Settings backup rejected: malformed data; no settings were changed.", true);
                return;
              }
              ...
              if (typeof confirm !== "function" || !confirm(summary)) {
                setFeedback("Settings import cancelled.", false);
                return;
              }
              const result = applySettingsBackup(raw, { hostname, storage: localStorage });
              if (!result.ok) {
                setFeedback("Settings import rejected; existing settings were restored.", true);
                return;
              }
            ...
            settingsFile.addEventListener("change", () => {
              ...
              if (!file || typeof file.text !== "function") {
                setFeedback("Choose a JSON backup file.", true);
                return;
              }
            ...
            const exportSettingsButton = makeButton("Export settings", "taa-settings-export", () => {
              ...
              if (includeWebhook && (typeof confirm !== "function" || !confirm("This export will contain your live Discord webhook secret as plain, readable JSON. Anyone with this file can post to your Discord channel. Export with the secret?"))) {
                setFeedback("Settings export cancelled; nothing was downloaded.", false);
                return;
              }
              const backup = buildSettingsBackup({ hostname, storage: localStorage, includeWebhook });
              previewBackup(backup);
              if (backup.bounded) {
                setFeedback("Settings backup exceeds the 512 KiB limit and was not downloaded.", true);
                return;
              }
