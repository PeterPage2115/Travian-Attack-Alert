  ...
  function initAdminPanel() {
    ...
    const renderWorkspace = () => {
      ...
      if (adminTab === "overview") {
        ...
        // T15 (T14 ledger): lead with plain-language monitoring status and next
        // action — setup/operational wording first (standby/leader, route, lease,
        // scan, baseline, delivery recovery pointer), then condensed status
        // metrics, then raw counters grouped behind one native disclosure.
        ...
        for (const line of buildOperationalStateLines({
          ...
          // T14 merge: the Session row folds into the Storage health metric; the
          // model still returns all eight lines, so its pinned deepEqual stays.
