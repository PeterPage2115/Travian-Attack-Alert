...
**Version 1.0.2, release ID `taa-1.0.2` — staged release candidate (`stable: false`).** This bump is staged off the release branch, so the live `release/public-1.0.0` channel keeps serving the published `1.0.1` stable release (tag `v1.0.1`, immutable GitHub Release, SHA-256 `0c187870…`); its historical record is archived at `docs/release-history/1.0.1/release-state.json`. Stable `1.0.2` publication requires owner pilot evidence; the machine-readable gate is `docs/release-state.json` (the 1.0.2 candidate) and the ordered publication procedure is `docs/RELEASE-RUNBOOK.md`.
...
**Install from the release-branch raw URL — the verified working path.** The channel URL below returned HTTP 200 and served exactly the release-branch artifact (SHA-256 `0c187870…`) when verified on 2026-09-23, and it still serves that build today — the `1.0.2` identity of this tree is staged off the release branch and is not published yet. No GitHub Release download exists yet, so install from this URL rather than waiting for one or trusting a mirror:
...
- **Pilot, not stable.** `docs/release-state.json` records `stable: false` for the staged `1.0.2` candidate with every owner gate unrecorded (including `ownerManual.branchProtection: false`), and there is no `v1.0.2` tag or GitHub Release. The published `1.0.1` (`stable: true`, tag `v1.0.1`, immutable GitHub Release) remains the live stable channel; its historical record is archived at `docs/release-history/1.0.1/release-state.json`. The public GitHub API observes branch protection with four required checks, but that observation is separate from the owner attestation.
