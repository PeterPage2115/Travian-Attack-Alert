...
- Jedno zrodlo prawdy: `package.json` (`version`, aktualnie 1.0.2).
...
- `src/lifecycle.js` (`createLifecycleController`) jest jedynym właścicielem mutowalnych singletonów cyklu życia; `src/adapters.js` to 7 fabryk seamu (storage, clock, sleep, gmRequest, docLoc, webLocks, sessionStore). `src/runtime.js` pozostaje legacy autorytetem (325 eksportów, release ID `taa-1.0.2`); produkcyjne wiring bez zmian. Granice wymusza `test/tools/module-architecture.test.cjs`.
