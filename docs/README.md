...
- Machine-readable release state (owner gates): [`release-state.json`](release-state.json) — the live `1.0.2` record (`stable: true`, every owner gate populated, `publication.tagAndRelease: true` for the published `v1.0.2` tag and immutable GitHub Release on 2026-09-25); the earlier published `1.0.1` record (`stable: true`, `publication.tagAndRelease` recording the published `v1.0.1` tag and immutable GitHub Release on 2026-09-25) is archived under release history.
...
- [`release-history/1.0.1/release-state.json`](release-history/1.0.1/release-state.json) — byte-identical archive of the published, owner-attested `1.0.1` release-state record (`stable: true`, `publication.tagAndRelease` recorded), kept as history while the live `release-state.json` records the published `1.0.2` stable release.
