...
- Machine-readable release state (owner gates): [`release-state.json`](release-state.json) — the live `1.0.2` candidate (`stable: false`, every owner gate still unrecorded); the published `1.0.1` record (`stable: true`, `publication.tagAndRelease` records the published `v1.0.1` tag and immutable GitHub Release on 2026-09-25) is archived under release history.
