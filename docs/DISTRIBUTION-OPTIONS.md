...
Status: **decision record only — no distribution change is performed or authorized by this page.** Recorded 2026-09-24 against identity `1.0.1` / `taa-1.0.1` (`package.json` as of that date; the published identity is now `1.0.2` / `taa-1.0.2`). This page generates no extension manifest, extension code, PWA code, dependency, storage migration, or release channel. It compares four shells for the existing runtime, cites the constraints to primary sources, marks the policy questions unresolved, and recommends keeping the tested Tampermonkey userscript as the current release artifact.
